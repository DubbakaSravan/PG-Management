...
function getAuth() {
  ...
  if (!raw) throw new Error("GOOGLE_SERVICE_ACCOUNT_JSON missing from environment");
  ...
  // Fix private key — Vercel sometimes escapes \n as \\n
...
export async function deleteRow(tab, rowNum) {
  const s = await getSheets();
  const meta = await s.spreadsheets.get({ spreadsheetId: SID() });
  const sheet = meta.data.sheets.find(sh => sh.properties.title === tab);
  if (!sheet) return;
  await s.spreadsheets.batchUpdate({ spreadsheetId: SID(), requestBody: { requests: [{ deleteDimension: { range: { sheetId: sheet.properties.sheetId, dimension: "ROWS", startIndex: rowNum - 1, endIndex: rowNum } } }] } });
}
